...
- If `publish_release` is set to `true`, the workflow creates or updates a GitHub **Release** with generated release notes and uploads packages, installer, and checksums as Release assets.
...
The script reads commits from the previous `v*` tag to the current workflow commit and groups them into:
...
The GitHub release category config is still kept for future PR-based workflows:

```text
.github/release.yml
```

If the project later moves to PR-based releases, labels such as these will make GitHub-native generated notes cleaner:
...
Direct pushes are supported by the project script, but commit messages should stay meaningful because they become release notes.
